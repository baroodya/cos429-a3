...
def check_layers():
    ...
    hyper = {"filter_size": 5, "filter_depth": 6, "num_filters": 7}
    ...
    if not (out.shape == out_.shape):
        ...
        print(
            "\tForward: %s"
            % pass_fail(
                (err_out if noflip else err_out_fl) < err_thresh
            )
        )
    ...
    if not (grad["W"].shape == grad_["W"].shape):
        ...
        err_grad = mse(
            np.concatenate(
                (grad["W"].reshape(-1), grad["b"].reshape(-1))
            ),
            np.concatenate(
                (grad_["W"].reshape(-1), grad_["b"].reshape(-1))
            ),
        )
        # compare flipped
        err_grad_fl = mse(
            np.concatenate(
                (grad_fl["W"].reshape(-1), grad_fl["b"].reshape(-1))
            ),
            np.concatenate(
                (grad_["W"].reshape(-1), grad_["b"].reshape(-1))
            ),
        ...
        print(
            "\tGradient: %s"
            % pass_fail(
                (err_grad if noflip else err_grad_fl) < err_thresh
            )
        )
